...
// Bold:POv2
  ...
  theme.Slideshow = (function() {
    ...
    function slideshow(el, args) {
      ...
      window.carousel = this.slideshow;
  ...
  theme.Photoswipe = (function() {
    var selectors = {
      ...
      images: '.mydiv_img',
      slideshowTrack: '.slick-track',
      activeImage: '.mydiv_img'
    };
...
jQuery(document).ready(function(){
  ...
  var photoshtm = jQuery(".sub_product__main-photos2").html();

  setTimeout(function(){

    jQuery(document).on('click', '.bold_option_swatch .bold_option_value', function(){
      //console.log(photoshtm);
      if(jQuery(window).width() < 767)
      {
        jQuery('.sub_product__main-photos2').slick('slickAdd',photoshtm);
      }
      var ths_input_val = jQuery(this).find('input').val();
      var input_nw_lowercase1 = ths_input_val.toLowerCase();
      jQuery('.class_wd').each(function(){
        var image_alt = jQuery(this).find('img').attr('alt');
        if(input_nw_lowercase1 == image_alt){
          jQuery(this).show();
          jQuery(this).addClass('div_act');
        }

        else{
          jQuery(this).hide();
          jQuery(this).removeClass('div_act');
          jQuery(this).removeAttr('data-i');
          jQuery(this).find('img').removeClass('mydiv_img');
        }

        if(jQuery(window).width() < 767)
        {
          if($(this).hasClass("div_act")){
          }
          else{
            $(this).remove();
          }
      ...
      if(jQuery(window).width() < 767)
      {
        $(".sub_product__main-photos2").slick('reinit');
      }

      //var img_src_n = [];


      jQuery('.div_act').each(function(i){
        jQuery(this).attr('data-i', i);
        var data_i_attr = jQuery(this).attr('data-i');
        jQuery(this).find('img').addClass('mydiv_img');

      });
    });
  }, 2000);
  setTimeout(function(){
    if($('.bold_option_swatch .bold_swatch_selected').length > 0){
      var ths_input_val1 = jQuery('.bold_option_swatch .bold_swatch_selected').parent().find('input').val();
      var input_nw_lowercase = ths_input_val1.toLowerCase();
      jQuery('.class_wd').each(function(){
        var image_alt1 = jQuery(this).find('img').attr('alt');

        if(input_nw_lowercase == image_alt1){
          jQuery(this).show();
          jQuery(this).addClass('div_act');
        }
        else{
  ...
  if(jQuery(window).width() < 767)
  {
    var img_alt_new = $('#mob-slide').find('img').attr('alt');
    if(img_alt_new != ""){
      setTimeout(function(){
        ...
        (function($) {
          var myCarousel = $(document.getElementById('mob-slide'));
          myCarousel.slick({
            infinite: false,
            arrows: false
          });
